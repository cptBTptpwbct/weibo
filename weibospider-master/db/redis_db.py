...
task_db = REDIS_ARGS.get('task', 10)
...
if sentinel_args:
    ...
    task_con = sentinel.master_for(master_name, socket_timeout=socket_timeout, db=task_db)
else:
    ...
    task_con = redis.Redis(host=host, port=port, password=password,db=task_db)
...
class Tasks(object):
    @classmethod
    def push_task(cls, type, id , index):
        task = json.dumps({'type': type, 'id': id, 'index': index})
        cls.push_in_queue(task)

    @classmethod
    def push_in_queue(cls, task):
        # if the concurrency is large, we can't guarantee there are no reduplicate values
        for i in range(task_con.llen('task_list')):
            tn = task_con.lindex('task_list', i)
            if tn:
                if tn == task:
                    return
        task_con.rpush('task_list', task)

    @classmethod
    def pop_out_queue(cls):
        return task_con.blpop('task_list')
