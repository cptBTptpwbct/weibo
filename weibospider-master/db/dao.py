...
class CommonOper:
    @classmethod
    @db_commit_decorator
    def add_one(cls, data, session=db_session):
        session.add(data)
        session.commit()

    @classmethod
    @db_commit_decorator
    def add_all(cls, datas, session=db_session):
        try:
...
class SeedidsOper(CommonOper):
    ...
    @classmethod
    @db_commit_decorator
    def insert_seeds(cls, ids, session = db_session):
        session.execute(SeedIds.__table__.insert().prefix_with('IGNORE'), [{'uid': i} for i in ids])
        session.commit()

    @classmethod
    @db_commit_decorator
    def set_seed_id(cls, seed, session = db_session):
        session.add(seed)
        session.commit()
...
class WbDataOper(CommonOper):
    @classmethod
    def get_wb_by_mid(cls, mid, session=db_session):
        return session.query(WeiboData).filter(WeiboData.weibo_id == mid).first()

    @classmethod
    def get_weibo_comment_not_crawled(cls, session):
        return session.query(WeiboData.weibo_id).all()

    @classmethod
    def get_weibo_praise_not_crawled(cls, session = db_session):
        return session.query(WeiboData.weibo_id).filter(text('praise_crawled=0')).all()

    @classmethod
    def get_weibo_repost_not_crawled(cls, session = db_session):
        return session.query(WeiboData.weibo_id, WeiboData.uid).filter(text('repost_crawled=0')).all()

    @classmethod
    def get_weibo_dialogue_not_crawled(cls, session):
        return session.query(WeiboData.weibo_id).filter(text('dialogue_crawled=0')).all()

    @classmethod
    @db_commit_decorator
    def set_weibo_comment_crawled(cls, mid,session):
    ...
    @classmethod
    @db_commit_decorator
    def set_weibo_praise_crawled(cls, mid,session):
        data = cls.get_wb_by_mid(mid)
        if data:
    ...
    @classmethod
    @db_commit_decorator
    def set_weibo_repost_crawled(cls, mid, session) :
        data = cls.get_wb_by_mid(mid)
        if data:
    ...
    @classmethod
    @db_commit_decorator
    def set_weibo_dialogue_crawled(cls, mid, session):
        data = cls.get_wb_by_mid(mid)
        if data:
...
class CommentOper(CommonOper):
    ...
    @classmethod
    def get_all_comment_by_weibo_id(cls, cid):
        return db_session.query(WeiboComment).filter(WeiboComment.weibo_id == cid).all()
